...
try:
    ...
    print(f"Erro: O arquivo {file_path} não foi encontrado. Verifique o caminho do arquivo.")
    exit()
...
# Agrupar por UA (UP) e Especie_campo para calcular volume total e número de árvores
grouped_volume = df.groupby(['UA', 'Especie_campo'])['Volume_m3'].sum().reset_index()
grouped_trees = df.groupby(['UA', 'Especie_campo']).size().reset_index(name='Número de Árvores')
...
bulletin.rename(columns={
    'UA': 'Unidade de Produção',
    'Especie_campo': 'Espécie',
...
# Exibir mensagem de confirmação e resultados
