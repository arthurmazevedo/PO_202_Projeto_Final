...
def aggregate_data(df):
    ...
    # Estimar custo variável por UP baseado em complexidade (ex: mais árvores = maior custo)
    num_arvores = up_group['Número de Árvores'].sum()
    custo_base_por_m3 = 150.0 + np.random.normal(0, 30, len(up_group))  # Variação aleatória para realismo (±20%)
...
def setup_parameters(V, R, area_total_ha=175.35, ciclo_anos=30, intensidade_sustentavel_ha=25):
    """Define parâmetros realistas baseados em práticas de manejo sustentável na Amazônia."""
    sum_V = V.sum()
    sum_R = R.sum()
    
    # Cálculo de metas sustentáveis com variação estocástica para realismo
    volume_sustentavel_total = area_total_ha * intensidade_sustentavel_ha * np.random.uniform(0.9, 1.1)  # ±10% variação
    frac_sustentavel = min(1.0, volume_sustentavel_total / sum_V)  # Não exceder estoque
    
    H = ciclo_anos
    T = list(range(1, H + 1))
    
    Vmeta = (sum_V * frac_sustentavel) / H  # ~1-1.5 m³/ha/ano equivalente
    Rmeta = (sum_R * frac_sustentavel) / H
    
    alpha = 0.35  # Aumentado para +35%/-35% tolerância, simulando flutuações de mercado/clima
    frac_max_anual_por_up = 0.25  # Max 25% por ano por UP (corte seletivo, com redução para realismo)
    taxa_desconto = np.random.uniform(0.08, 0.12)  # Variação em taxa de desconto (8-12%)
    variacao_minima = 0.08  # Forçar pelo menos 8% de variação ano a ano para realismo
    penalidade_fator = 0.05  # Aumentado para penalizar mais desvios e induzir trade-offs
...
def build_model(U, T, V, R, custo_por_up, Vmeta, Rmeta, alpha, frac_max_anual_por_up, taxa_desconto, variacao_minima, penalidade_fator):
    ...
    prob = pulp.LpProblem("Manejo_Florestal_Sustentavel_Realista", pulp.LpMaximize)
    ...
    # Variáveis para forçar variação (desvios absolutos entre anos consecutivos)
    delta_volume = pulp.LpVariable.dicts("delta_volume", [t for t in T if t > 1], lowBound=0, cat='Continuous')
    
    # Função-objetivo: Maximizar Valor Presente Líquido (VPL) com penalidade por desvios e custos ambientais
    ...
    penalidade_variacao = 0.02 * pulp.lpSum([delta_volume[t] for t in [tt for tt in T if tt > 1]])  # Penaliza variações excessivas
    prob += vpl - penalidade_desvio - penalidade_variacao, "Max_VPL_Com_Equilibrio_Realista"
    ...
    for t in T:
        ...
        # Faixas realistas para volume e receita com assimetria (mais tolerância para baixo)
        prob += volume[t] >= Vmeta * (1 - alpha * 1.2), f"Volume_Min_{t}"  # Mais flexível para baixo
        ...
        # Restrição de lucratividade mínima por ano (lucro > 60% da receita)
        prob += receita[t] - custo[t] >= 0.6 * receita[t], f"Lucratividade_Min_{t}"
    
    # Forçar variação mínima entre anos consecutivos para realismo
    for t in T:
        if t > 1:
            ...
            prob += delta_volume[t] <= variacao_minima * Vmeta * 3, f"Variacao_Max_{t}"  # Limite superior para estabilidade
    ...
    for u in U:
        ...
        for t in T:
            # Intensidade anual máxima
            prob += x[u][t] <= frac_max_anual_por_up, f"Intensidade_Anual_{u}_{t}"
            
            # Evitar colheitas consecutivas totais (período mínimo de regeneração: 5 anos sem corte pleno)
            for k in range(1, 5):  # Não colher >10% nos 4 anos seguintes
                if t + k <= len(T):
...
def solve_and_report(prob, U, T, V, R, Vmeta, Rmeta, output_file='resultados_manejo_realista.csv'):
    """Resolve o modelo, diagnostica problemas e gera relatório."""
    solver = pulp.PULP_CBC_CMD(msg=1, timeLimit=1200, gapRel=0.03)  # Aumentado tempo e gap para complexidade
    ...
    if status != 1:
        ...
        for constraint in list(prob.constraints.values()):
            if 'Regeneracao' in constraint.name or 'Variacao_Max' in constraint.name:
                prob -= constraint  # Remover temporariamente
        status = prob.solve(solver)
    ...
    print("\nResumo Anual de Manejo Florestal Sustentável Realista:")
    print(df_results.to_string(index=False))
    
    return df_results

# Comando principal para executar o programa
if __name__ == "__main__":
    file_path = 'wood_volume_value_bulletin_acre.csv'  # Comando para ler o arquivo CSV
    try:
